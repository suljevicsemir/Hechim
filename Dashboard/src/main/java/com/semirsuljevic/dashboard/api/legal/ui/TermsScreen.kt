...
@Composable
fun TermsScreen( viewModel: LegalViewModel = hiltViewModel() ) {
    ...
    HechimScreen (
        config = HechimScreenConfig(
            title = UiText.StringResource(R.string.terms_page_title).asString()
        ),
        resource = viewModel.terms
    ){
        Column (
            modifier = Modifier
                .padding(it)
